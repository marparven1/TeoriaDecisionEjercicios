...
criterio.Wald = function(tablaX,favorable=TRUE) {
  ...
  X = tablaX;
  if (favorable) {
    AltW = apply(X,MARGIN=1,min); # Mínimos por filas (Beneficio)
    ##AltW
    Wald = max(AltW); # El máximo de los mínimos
    Alt_Wald = which.max.general(AltW); # En que índice se alcanza ese máximo (General porque puede haber empates)
    metodo = 'favorable';
  } else { # Lo contratio
    AltW = apply(X,MARGIN=1,max); # El máximo beneficio
    ##AltW
    Wald = min(AltW); # Cual es el mínimo del máximo beneficio
    Alt_Wald = which.min.general(AltW); # índice
    metodo = 'desfavorable';
  }
  resultados = list();
